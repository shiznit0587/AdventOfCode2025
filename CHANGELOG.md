...
I then ran into all kinds of problems with using zig's linked list types. It's API is weird - itstead of being implemented with a generic node struct containing data, it requires your data to have a node member, then you have to use the `@fieldParentPtr` built-in to access the data containing the node. Once I got past the API, I was hitting use-after-free issues. If I block-allocated a bunch of items for the queue, freeing one of them would free them all from that block. There was absolutely no error messaging or handling to help me figure this out.

For the problem itself, I stored the lights state and button modifier as single values, and used XOR operator to apply the button to the lights, and I did intuit that I'd never want to press the same button twice. But I kept failing on the sample input. After lots of debugging, I realized I was parsing the lights in backwards. The solution took over 8s to run. I want to find a cleaner approach. I found that my input has at most 13 buttons. I wonder if I can do a recursive solution instead of a queue-based one.
